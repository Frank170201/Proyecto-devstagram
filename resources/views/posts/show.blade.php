...
@section('contenido')
    <div class="container mx-auto md:flex" style="padding: 3%">
        <div class="md:w-1/2">
                ...
                @if ($post->user_id === auth()->user()->id)
                    <form method="POST" action="{{ route('posts.destroy', $post) }}">
                        ...
                        <input type="submit" value="Eliminar Publicacion"
                            class="bg-red-600 p-2 rounded text-white font-bold mt-4 cursos-pointer">
                    </form>
                @endif
            @endauth
        </div>
            ...
            <div class="shadow bg-white p-5 mb-5 ">
                ...
                <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                    @if ($post->comentarios->count())
                        ...
                        @endforeach
                    @else
                        <p class="p-10 text-center">No hay comentacios aun</p>
                    @endif
                </div>
            </div>
        </div>
